...
import { Radio, Cpu, Radar, Shield, Compass, ArrowRight, Anchor } from "lucide-react";
import VideoPlaceholder from "../components/VideoPlaceholder";
import ChartEyebrow from "../components/ChartEyebrow";
import HeroChart from "../components/HeroChart";

const ACTIVITES = [
  { icon: Anchor, title: "Placement d'équipage USV", description: "Recrutement et mise à disposition d'opérateurs télépilotes, superviseurs de mission et techniciens pour votre flotte.", href: "/activites" },
  ...
  { icon: Shield, title: "Conseil & structuration RH", description: "Fiches de poste, référentiels internes, accompagnement réglementaire — avant de recruter.", href: "/employeurs" },
];
...
export default function Home() {
  return (
    <main className="bg-[#FAFBFC] text-[#0B2239]">
      <section className="chart-grid border-b border-[#0B2239]/10">
        <div className="px-6 lg:px-16 py-16 lg:py-24 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <div className="flex flex-wrap gap-4">
              <Link href="/candidature" className="flex items-center gap-2 font-chart text-[11px] uppercase tracking-widest bg-[#F4530B] text-white px-6 py-3.5 rounded-md hover:bg-[#d94806] transition-colors">
      ...
      <section className="px-6 lg:px-16 py-16">
        <div className="rounded-lg overflow-hidden aspect-[21/9] relative">
          <img src="https://images.unsplash.com/photo-1651916850717-1f91cb32ec2c?fm=jpg&q=80&w=2000&auto=format&fit=crop" alt="Vue aérienne d'un navire en mer" className="w-full h-full object-cover" />
          ...
          <p className="absolute bottom-4 left-5 font-chart text-[10px] uppercase tracking-widest text-white/70">
            Zone d&apos;opération · Méditerranée — Europe
          </p>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <p className="text-[#0B2239]/60 leading-relaxed">
              ...
              défense : les usages se multiplient, et avec eux le besoin de personnel
      ...
      <section className="px-6 lg:px-16 py-16 border-t border-[#0B2239]/10">
        ...
        <h2 className="font-display text-2xl lg:text-3xl font-semibold mb-12 max-w-xl">
          Trois activités, une seule mission : armer les flottes autonomes.
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {ACTIVITES.map((a) => {
            const Icon = a.icon;
            return (
              <Link key={a.title} href={a.href} className="bg-white border border-[#0B2239]/10 rounded-lg p-6 hover:border-[#F4530B]/60 hover:-translate-y-0.5 transition-all group block">
                <Icon className="w-5 h-5 text-[#0E7490] mb-5" strokeWidth={1.5} />
      ...
      <section className="px-6 lg:px-16 py-16 border-t border-[#0B2239]/10 bg-white">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-8">
          {COMPETENCES.map((c) => {
            const Icon = c.icon;
            return (
              <div key={c.label} className="flex items-center gap-3 border border-[#0B2239]/10 bg-[#FAFBFC] rounded-md px-4 py-4">
      ...
      <section className="chart-grid px-6 lg:px-16 py-20 border-t border-[#0B2239]/10 text-center">
        ...
        <p className="text-[#0B2239]/60 mb-8 max-w-xl mx-auto">
          Déposez votre profil — nous le mettons en relation avec les opérateurs de flotte qui recrutent.
        </p>
        <Link href="/candidature" className="inline-flex items-center gap-2 font-chart text-[11px] uppercase tracking-widest bg-[#F4530B] text-white px-7 py-4 rounded-md hover:bg-[#d94806] transition-colors">
